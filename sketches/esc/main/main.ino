...
void loop() {
  ...
  Mctrl.motor_move(Boal_RT.theta,70,Mom_now);
  }
  */
  //回り込みなどを書き足したもの
  if(flag){
    ...
    if((Boal_RT.radius > 100) | true){
      if(true)Mctrl.motor_move(Boal_RT.theta,0,-50);
      else if(-PI <= Boal_RT.theta && Boal_RT.theta <= -7*PI/12)Mctrl.motor_move(-PI/2,power,Mom_now);
      ...
      if(Ultrasonic_back.measure() > 25)Mctrl.motor_move(3*PI/2,power,Mom_now);
      else Mctrl.motor_stop();
    }
  }
